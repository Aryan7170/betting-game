...
contract NewBettingGameIntegrationTest is Test {
    ...
    function testScriptInteractionIntegration() public {
        // Test integration with interaction scripts
        PlaceBet placeBetScript = new PlaceBet();
        FulfillRandomWords fulfillScript = new FulfillRandomWords();
        
        // Place bet using script
        placeBetScript.placeCoinBet(address(bettingGame), 0.1 ether, 0, player1);
        
        // Verify bet was placed
        assertEq(bettingGame.nextBetId(), 1);
        
        // Fulfill using script
        fulfillScript.fulfillRandomWords(1, 0, address(mockVRFCoordinator), address(bettingGame), owner);
